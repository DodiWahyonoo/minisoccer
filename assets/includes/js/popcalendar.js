	var language = 'en';	// Default Language: en - english ; es - spanish; de - german ; ru - russian
	var enablePast = 1;		// 0 - disabled ; 1 - enabled
	var enableWeekdays = 1; // 0 - disabled ; 1 - enabled
	var fixedX = -1;		// x position (-1 if to appear below control)
	var fixedY = -1;		// y position (-1 if to appear below control)
	var startAt = 0;		// 0 - sunday ; 1 - monday
	var showWeekNumber = 0;	// 0 - don't show; 1 - show
	var showToday = 1;		// 0 - don't show; 1 - show
	var imgDir = 'images/date/';		// directory for images ... e.g. var imgDir="/img/"
	var dayName = '';
	...
	var     monthName = {
			en : new Array('Januari','Febuari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'),
	...
	if (startAt==0) {
			dayName = {
					en : new Array('Mg','Sen','Sel','Rab','Kam','Jum','Sab'),
			...
			dayName = {
					en : new Array('Sen','Sel','Rab','Kam','Jum','Sab','Mg'),
	...
	*/
	...
	function constructDate(d,m,y) {
		...
		sTmp = sTmp.replace ('<d>',d);
		sTmp = sTmp.replace ('<n>', dayName[language][selDate.getDay()-1]);
	...
	function constructCalendar () {
		...
		for	( datePointer=1; datePointer <= numDaysInMonth; datePointer++ ) {
			...
			//////////////////////////////////////////////
			if ((!enableWeekdays&&((dayPointer%7==nSat)||(dayPointer%7==nSun)))||((enablePast == 0) && ((yearSelected < yearNow) || (monthSelected < monthNow) && (yearSelected == yearNow) || (datePointer < dateNow) && (monthSelected == monthNow) && (yearSelected == yearNow)))) {
				selDayAction = '';
				isPast = 1;
			} else {
